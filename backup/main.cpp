...
// 每个传感器的回调函数（位于read_and_publish_sensor函数之后）
void timer_mag_callback_0(rcl_timer_t *timer, int64_t last_call_time) {
  Serial.println(" enter the timer_mag_callback_0");
  if (timer == NULL) return;
...
void timer_mag_callback_1(rcl_timer_t *timer, int64_t last_call_time) {
  if (timer == NULL) return;
  Serial.println(" enter the timer_mag_callback_1");
...
void timer_mag_callback_3(rcl_timer_t *timer, int64_t last_call_time) {
  Serial.println(" enter the timer_mag_callback_3");
  if (timer == NULL) return;
...
void setup()
{
  ...
  for (int i = 0; i < SENSOR_COUNT; i++) {
    ...
    // Serial.printf("magnetic_sensor_%d\n", i);
  ...
  for (int i = 0; i < SENSOR_COUNT; i++) {
    if (!sensors[i].begin_I2C(sensor_addresses[i])) {
      ...
      // 设置分辨率、过采样和滤波
      sensors[i].setResolution(MLX90393_X, MLX90393_RES_17);
      sensors[i].setResolution(MLX90393_Y, MLX90393_RES_17);
      sensors[i].setResolution(MLX90393_Z, MLX90393_RES_16);
      sensors[i].setOversampling(MLX90393_OSR_3);
      sensors[i].setFilter(MLX90393_FILTER_5);
    }
  }

  // 校准数据部分保留原代码
  // 注意：理想情况下，每个传感器应有自己的校准数据
